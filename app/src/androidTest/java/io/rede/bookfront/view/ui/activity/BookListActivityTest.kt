...
class BookListActivityTest {
    ...
    @get:Rule
    var mActivityTestRule = ActivityTestRule(BookListActivity::class.java, true, false)
    ...
    fun testSuccessfulResponse() {
        mMockWebServer.dispatcher = object : Dispatcher() {
            override fun dispatch(request: RecordedRequest): MockResponse {
                return MockResponse()
                    .setResponseCode(200)
                    .setBody(FileReader.readStringFromFile("book_success_response.json"))
            }
        }

        mActivityTestRule.launchActivity(null)

        onView(withId(R.id.book_recyclerview))
